...
async function performSearch() {
    ...
    const rawKeywords = clipboardSearch.value.trim();

    if (!rawKeywords) {
        loadClipboards(); // fallback to all clipboards if empty
        return;
    }

    // Replace spaces with %20 (exactly as requested)
    const encodedKeywords = rawKeywords.split(' ').join('%20');

    container.innerHTML = '<div class="loading">Searching clipboards...</div>';

    try {
        const clipboards = await api.searchClipboards(encodedKeywords);

        if (!clipboards || clipboards.length === 0) {
            container.innerHTML = '<p class="loading">No matching clipboards found.</p>';
            return;
        }
...
`).join('');
    ...
    document.querySelectorAll('.clipboard-card').forEach(card => {
        card.addEventListener('click', () => {
            showClipboardDetails(card.dataset.id);
        });
    });
}

// Initialization is handled in the authentication check at the top of the file